    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                logger.warning(
                    f"⏰ Timeout in circuit breaker '{self.name}' (attempt {attempt + 1}/{self.config.max_retries + 1})"
                )

            except Exception as e:
                last_exception = e
                logger.warning(
                    f"❌ Error in circuit breaker '{self.name}' (attempt {attempt + 1}/{self.config.max_retries + 1}): {str(e)}"
                )
        ...
        with self._lock:
            ...
            if self.state == CircuitState.CLOSED:
                if self.failure_count >= self.config.failure_threshold:
                    logger.error(
                        f"🔴 Circuit breaker '{self.name}' OPEN - failure threshold exceeded "
                    ...
                    logger.debug(
                        f"⚠️ Failure #{self.failure_count} for '{self.name}' "
