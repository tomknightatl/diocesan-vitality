...
def _extract_address_pattern_from_raw_name(raw_name: str) -> tuple[str, str]:
    ...
    return potential_address, parish_name


def _build_street_address_from_components(parsed_components: dict) -> str:
    """Build street address from parsed address components."""
    street_parts = []

    # Build street address from components in order
    component_keys = [
        "AddressNumber",
    ...
    for key in component_keys:
        if parsed_components.get(key):
            street_parts.append(parsed_components[key])
...
def enhanced_address_parsing(raw_name: str) -> Dict:
    ...
    try:
        ...
        street_address = _build_street_address_from_components(parsed_components)
    ...
    def _determine_extraction_strategy(
        ...
        if listing_type == ParishListingType.IFRAME_EMBEDDED:
            ...
            return (
                ...
                0.8,
                {
                    "items": ".summary - item, .parish - item",
                    "title": ".summary - title",
                },
...
def _validate_upsert_prerequisites(supabase, parishes):
    """Validate prerequisites for upsert operation."""
    if not supabase:
    ...
    if not parishes:
        ...
        return True, "No parishes to process"
...
def _filter_and_prepare_parishes(
    ...
    for parish in parishes:
        try:
            ...
            if not parish.name or len(parish.name.strip()) < 3:
                logger.debug("    ⏭️ Skipped: Invalid name for parish")
                skipped_count += 1
                continue
            ...
            if not clean_data.get("Name") or len(clean_data.get("Name", "")) < 3:
                logger.debug("    ⏭️ Skipped: Invalid name after cleaning")
                skipped_count += 1
                continue
    ...
    return valid_parishes, skipped_count, detail_success_count


def _prepare_batch_data_and_actions(batch, supabase, monitoring_client, diocese_id):
    """Prepare batch data and determine parish actions (add/update)."""
    batch_data = []
    parish_actions = {}
...
def _log_successful_batch(batch, parish_actions, monitoring_client, diocese_name):
    """Log successful batch processing results."""
...
def enhanced_safe_upsert_to_supabase(
    ...
    is_valid, message = _validate_upsert_prerequisites(supabase, parishes)
    if not is_valid:
        return False
    if message == "No parishes to process":
        return True

    # Phase 2: Filter and prepare data for batch processing
    valid_parishes, skipped_count, detail_success_count = _filter_and_prepare_parishes(
        parishes, diocese_id, diocese_name, diocese_url, parish_directory_url
    )
    ...
    for batch_num in range(total_batches):
        ...
        batch_success = _process_single_batch(
            batch_num,
            total_batches,
            batch,
            supabase,
