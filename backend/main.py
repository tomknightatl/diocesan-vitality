...
async def get_chart(chart_name: str):
    """Serve chart images generated by the reporting worker"""
    # Validate chart name to prevent directory traversal
    allowed_charts = [
    ...
    # Charts are saved to /app/charts by the pipeline reporting worker
    chart_path = Path(f"/app/charts/{chart_name}")

    if not chart_path.exists():
        raise HTTPException(status_code=404, detail=f"Chart not yet generated: {chart_name}")
