...
async def lifespan(app: FastAPI):
    ...
    print("📊 WebSocket monitoring available at: /ws/monitoring")
    print("🔧 API monitoring endpoints available at: /api/monitoring/*")
...
def get_dioceses(
    ...
    try:
        ...
        dir_url_map, blocking_data_map = _fetch_directory_and_blocking_data()
        parish_counts, parishes_with_data_counts = _fetch_parish_counts()

        # Enrich dioceses with additional data
        _enrich_dioceses_with_additional_data(
            dioceses,
            dir_url_map,
            blocking_data_map,
            parish_counts,
            parishes_with_data_counts,
        )

        # Sort and paginate
        dioceses = _sort_dioceses(dioceses, sort_by, sort_order)
...
def _fetch_and_filter_dioceses(filter_name, filter_address, filter_website):
    """Fetch dioceses and apply filters."""
...
def _fetch_directory_and_blocking_data():
    """Fetch directory URLs and blocking detection data."""
    all_dir_response = (
    ...
    return dir_url_map, blocking_data_map


def _fetch_parish_counts():
    """Fetch parish counts and data extraction counts."""
...
def _sort_dioceses(dioceses, sort_by, sort_order):
    """Sort dioceses by specified field and order."""
...
def _apply_basic_filters(query, filter_name, filter_diocese_name, filter_website, filter_blocked):
    ...
    if filter_website:
        query = query.ilike("Web", f"%{filter_website}%")
    if filter_blocked:
...
def _get_data_extraction_filter_ids(filter_data_extracted, filter_data_available, diocese_id=None):
    ...
    return filter_parish_ids


def _apply_sorting_and_pagination(query, sort_by, sort_order, offset, page_size):
    """Apply sorting and pagination to the parish query."""
    if sort_by == "Name":
...
def _enrich_parishes_with_data(parishes):
    """Enrich parishes with fact data and blocking information."""
    if not parishes:
        return parishes
    ...
    # Create lookup for parish facts
    parish_facts = {}
...
def _apply_post_query_filters_and_sorting(
    parishes,
    filter_data_extracted,
...
def _apply_blocking_filters(parishes: list, filter_blocked: str) -> list:
    """Apply blocking status filters"""
    if filter_blocked:
        if filter_blocked.lower() == "true":
...
def _apply_computed_field_sorting(parishes: list, sort_by: str, sort_order: str) -> list:
    ...
    if sort_by == "data_extracted":
        parishes.sort(
            key=lambda p: p.get("data_extracted", False),
            reverse=sort_order.lower() == "desc",
        )
    elif sort_by == "is_blocked":
        parishes.sort(
            key=lambda p: p.get("is_blocked", False),
            reverse=sort_order.lower() == "desc",
        )
...
def get_parishes_for_diocese(
    ...
    try:
        ...
        # Enrich parishes with fact data
        parishes = _enrich_parishes_with_data(parishes)

        # Apply post - query filters and sorting
        parishes = _apply_post_query_filters_and_sorting(
            parishes,
            filter_data_extracted,
            filter_data_available,
            filter_blocked,
...
def get_all_parishes(
    ...
    try:
        ...
        if parishes:
            parishes = _enrich_parishes_with_data_flags(parishes)
            parishes = _normalize_parish_fields(parishes)
...
def _build_parishes_query(
    filter_name: str,
    filter_diocese_name: str,
    filter_website: str,
    filter_blocked: str,
):
    """Build base query with basic filters"""
...
def _apply_data_extraction_query_filters(
    query,
    filter_data_extracted: str,
    filter_data_available: str,
    page: int,
    page_size: int,
):
...
def _enrich_parishes_with_data_flags(parishes: list) -> list:
...
def _normalize_parish_fields(parishes: list) -> list:
    """Normalize parish fields for frontend compatibility"""
    for parish in parishes:
        # Map database column names to frontend expectations
        if "Web" in parish:
            parish["Website"] = parish["Web"]
