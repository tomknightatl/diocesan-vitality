...
echo -e "${BLUE}🚀 Deploying USCCB Pipeline to Kubernetes${NC}"
...
echo "Container Registry: docker.io/$GITHUB_USERNAME/usccb:pipeline"

# Build and push Docker image
echo -e "${BLUE}🔨 Building Docker image...${NC}"
docker build -f Dockerfile.pipeline -t $GITHUB_USERNAME/usccb:pipeline .

echo -e "${BLUE}📤 Pushing to Docker Hub...${NC}"
docker push $GITHUB_USERNAME/usccb:pipeline

# Update Kubernetes manifests with correct image name
echo -e "${BLUE}🔄 Updating Kubernetes manifests...${NC}"
sed -i.bak "s/tomknightatl\/usccb-pipeline:latest/$GITHUB_USERNAME\/usccb:pipeline/g" k8s/pipeline-cronjob.yaml
sed -i.bak "s/tomknightatl\/usccb-pipeline:latest/$GITHUB_USERNAME\/usccb:pipeline/g" k8s/pipeline-job.yaml
...
if ! kubectl get secret usccb-secrets -n usccb &> /dev/null; then
    echo -e "${YELLOW}⚠️  Secret 'usccb-secrets' not found in usccb namespace. Please create it manually:${NC}"
    echo "kubectl create secret generic usccb-secrets -n usccb \\"
...
echo "• View Logs: kubectl logs -l job-name=usccb-pipeline-cronjob-XXXXX"
echo "• Run manual job: kubectl apply -f k8s/pipeline-job.yaml"
echo "• Delete job: kubectl delete job usccb-pipeline-job"