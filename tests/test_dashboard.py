...
from core.monitoring_client import ExtractionMonitoring, MonitoringClient, get_monitoring_client
                ...
                for i in range(total_parishes):
                    ...
                    if random.random() < 0.85:  # 85% success rate
                        ...
                        if random.random() < 0.3:  # 30% chance of detailed log
                            client.send_log(f"✅ Extracted parish {i+1}: Enhanced data found", "INFO")
                    else:
                        client.send_log(f"⚠️ Parish {i+1}: Partial data extracted", "WARNING")
...
async def continuous_monitoring_demo():
    ...
    for cycle in range(10):  # Run for 10 cycles (about 5 minutes)
        try:
            ...
            activity_type = random.choice(["parish_validation", "data_cleanup", "schedule_extraction", "address_parsing"])
...
def test_basic_monitoring():
    ...
    client.report_error(
        error_type="TestError", message="This is a test error for dashboard validation", diocese="Test Diocese"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["basic", "extraction", "continuous"], default="extraction", help="Test mode to run")
