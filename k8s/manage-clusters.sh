...
PRODUCTION_CLUSTER="do-nyc2-dv-prd"
STAGING_CLUSTER="do-nyc2-dv-stg"
DEV_CLUSTER="do-nyc2-dv-dev"
REGION="nyc1"
...
create_cluster_like_production() {
    ...
    # Parse the first (and likely only) node pool from production
    local first_node_pool=$(echo "$PROD_NODE_POOLS" | head -1)
    local node_size=$(echo "$first_node_pool" | awk '{print $2}')
    local node_count=$(echo "$first_node_pool" | awk '{print $3}')
    local node_tags=$(echo "$first_node_pool" | awk '{for(i=4;i<=NF;i++) printf "%s,", $i}' | sed 's/,$//')
    ...
    create_cmd="$create_cmd --node-pool \"name=${environment}-pool;size=${node_size};count=${node_count}\""

    if [[ -n "$PROD_VPC" && "$PROD_VPC" != "<no value>" ]]; then
        create_cmd="$create_cmd --vpc-uuid $PROD_VPC"
    fi

    if [[ -n "$PROD_TAGS" && "$PROD_TAGS" != "<no value>" ]]; then
        # Add environment-specific tag
        local env_tags="${PROD_TAGS},environment:${environment}"
        create_cmd="$create_cmd --tag \"$env_tags\""
    else
        create_cmd="$create_cmd --tag \"environment:${environment}\""
    fi
    ...
    if eval "$create_cmd"; then
        ...
        if doctl kubernetes cluster kubeconfig save "$new_cluster_name"; then
            ...
            echo "⚠️  Failed to save kubectl credentials. Run manually: doctl kubernetes cluster kubeconfig save $new_cluster_name"
        fi
...
main() {
    ...
    if kubectl config get-contexts | grep -q "$PRODUCTION_CLUSTER"; then
        ...
        echo "⚠️  kubectl context '$PRODUCTION_CLUSTER' not found"
